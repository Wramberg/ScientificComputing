...
\section{Phase error}
To compute the phase error between the in- and output phases, the function in \cref{code:phaseerr} was developed. This function return the error signal as well as the root mean square (RMS) error.
...
def getphaseerror(phix, phiy):
    try:
        ...
        raise Exception('Failed to compare phase signals, phiy signal cannot be longer than phix signal')
    rmserr = np.sqrt(np.mean(diff[250:] ** 2))
    return diff, rmserr
\end{lstlisting}
...
\section{Psd of in- and output (1)}
On \figref{fig:psd1600} the PSDs are presented for the in- and output signals.

\fig[keepaspectratio=true,width=\textwidth]{psds30k}{psd1600}{fig:psd1600}

As seen, the energy in each signal is located around...

\section{Time- and phase-offset (2)}
\fig[keepaspectratio=true,width=\textwidth]{phasesignals1k6}{psd1612300}{fig:psdqwd1600}
\fig[keepaspectratio=true,width=\textwidth]{phasesignals3k}{psd1612300}{fig:psd1qwd600}

\section{Time- and phase-errors 3}
\fig[keepaspectratio=true,width=\textwidth]{phasesignalscomp1k6}{psd1612300}{fig:psd1qwd600}
\fig[keepaspectratio=true,width=\textwidth]{phasesignalscomp3k}{psd1612300}{fig:psd1qwd600}

\section{rms and error stuff 4 og 5}
RMS = \\
0.0412276521009\\
RMS deg =\\
2.36217046462\\
\fig[keepaspectratio=true,width=\textwidth]{phaseerr3k}{psd1612300}{fig:psd1qwd600}
